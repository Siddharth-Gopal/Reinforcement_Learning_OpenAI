...
class DQAgent():
    ...
    def __init__(self):
        ...
        self.replay_mem = ReplayMemoryPER(50000)
    ...
    def random_play(self, num_episodes):
        for ep in range(num_episodes):
            state = self.env.reset()

            while True:
                action = np.random.randint(self.num_actions)
                new_state, reward, termination, info = self.env.step(action)
        ...
        print(num_episodes, ' episodes played and added to the replay memory')
    ...
    def train_model_mountain(self):
        ...
        # indices_to_train = self.replay_mem.sample(self.batch_size)
